...
const formEstadoVideo = document.getElementById("formEstadoVideo");
const btns = document.querySelectorAll('button[name="accion"]');
let estadoNombre = document.getElementById("estado_nombre");
//console.log(btns);

//GESTION DE ACCION SOBRE BOTONES AGREGAR, MODIFICAR
totalListEstadoVideo();
    ...
    btnElement.addEventListener('click', ()=>{
      ...
      switch (btnElement.value) {
        ...
        case "Agregar":
          ...
          if (estadoNombre.value === "") {
            alert("Escriba el nombre de un estado de video");
          } else if (estadoNombre.value != "") {
            let XHR = new XMLHttpRequest();
            XHR.open("POST", formEstadoVideo.action);
            let dataForm = new FormData(formEstadoVideo);
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              if (estadoNombre.value === "") {
                ...
                let dataForm = new FormData(formEstadoVideo);
                dataForm.append("btnValue", btnElement.value);
                dataForm.append("estadoId", estadoId.value);
                XHR.send(dataForm);
                XHR.onreadystatechange = function () {
                  let rpta = XHR.responseText;
                  // const rptaData = document.getElementById("rpta");
...
btnCancelar.addEventListener("click", totalListEstadoVideo);

//Mostrar toda la data tipo de video
const btnMostrarTodo = document.getElementById("btnMostrarTodo");
btnMostrarTodo.addEventListener("click", totalListEstadoVideo);

function totalListEstadoVideo() {
  let objXHR;
  ...
  objXHR.open(
    "GET",
    "/admin/practicas/mi_app/acciones/estado_video/get_estado_video.php",
    true
  );
  objXHR.send();
  objXHR.onreadystatechange = function () {
    if (objXHR.readyState == 4 && objXHR.status == 200) {
      ...
      //console.log(data);
      const estadoVideos = JSON.parse(data);
      let template = "";
      estadoVideos.forEach((estadoVideo) => {
        template += `
                  <tr class="fila">
                    <td>${estadoVideo.id}</td>
                    <td><a>${estadoVideo.estadoNombre}</a></td>
      ...
      let printEstadoVideos = document.getElementById("tabla_estado_video");
      printEstadoVideos.innerHTML = "";
      printEstadoVideos.innerHTML = template;
      resaltarFila();
    }
  };
}
...
function seleccionarRegistro(filaID) {
  const inputId = document.getElementById("estado_id");
  inputId.setAttribute("type", "hidden");
  let XHR = new XMLHttpRequest();
  XHR.open("POST", "/admin/practicas/mi_app/acciones/estado_video/get_id_estado.php");
  XHR.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  XHR.send(`estado_id=${filaID}`);
  XHR.onreadystatechange = function () {
    let rpta = XHR.responseText;
...
function eliminarRegistro(filaID) {
  let XHR = new XMLHttpRequest();
  XHR.open("POST", "/admin/practicas/mi_app/acciones/estado_video/eliminaEstadoVideo.php");
  XHR.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  XHR.send(`estado_id=${filaID}`);
  XHR.onreadystatechange = function () {
    let rpta = XHR.responseText;
...
function getRegistro(rptJSON) {
  let inputId = document.getElementById("estado_id");
  let inputEstadoNombre = document.getElementById("estado_nombre");

  rptJSON.forEach((rpt) => {
    inputId.value = rpt.id;
    inputEstadoNombre.value = rpt.estadoNombre;
  });
}
...
function limpiarInput() {
  const estadoNombre = document.getElementById("estado_nombre");
  estadoNombre.value = "";
}
