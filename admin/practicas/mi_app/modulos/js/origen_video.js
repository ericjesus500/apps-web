...
function totalListOrigenVideo() {
  ...
  objXHR.open(
    "GET",
    "/admin/practicas/mi_app/acciones/origen_video/get_origen_video.php",
    true
  );
  objXHR.send();
...
function seleccionarRegistro(filaID) {
  ...
  XHR.open(
    "POST",
    "/admin/practicas/mi_app/acciones/origen_video/get_id_origen_video.php"
  );
...
function eliminarRegistro(filaID) {
  ...
  XHR.open(
    "POST",
    "/admin/practicas/mi_app/acciones/origen_video/eliminaOrigenVideo.php"
  );
  XHR.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  XHR.send(`origen_id=${filaID}`);
  XHR.onreadystatechange = function () {
