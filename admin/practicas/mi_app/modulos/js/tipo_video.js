...
const formTipoVideo = document.getElementById('formTipoVideo');
const btns = document.querySelectorAll('button[name="accion"]');
let tipoVideoNombre = document.getElementById("tipo_video_nombre");
//console.log(btns);
    ...
    btnElement.addEventListener("click", () => {
      ...
      switch (btnElement.value) {
        ...
        case "Agregar":
          ...
          if (tipoVideoNombre.value === "") {
            alert("Escriba el nombre de un tipo de video");
          } else if (tipoVideoNombre.value != "") {
            let XHR = new XMLHttpRequest();
            XHR.open("POST", formTipoVideo.action);
            let dataForm = new FormData(formTipoVideo);
        ...
        case "Modificar":
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              const tipoId = document.getElementById("tipo_video_id");
              if (tipoVideoNombre.value === "") {
                alert("Escriba el nombre de un tipo de video");
              } else if (tipoVideoNombre.value != "") {
                let XHR = new XMLHttpRequest();
                ...
                let dataForm = new FormData(formTipoVideo);
                dataForm.append("btnValue", btnElement.value);
                dataForm.append("tipoId", tipoId.value);
                XHR.send(dataForm);
                XHR.onreadystatechange = function () {
                  let rpta = XHR.responseText;
                  const rptaData = document.getElementById("rpta");
...
btnCancelar.addEventListener('click', totalListTipoVideo)

//Mostrar toda la data tipo de video
const btnMostrarTodo = document.getElementById('btnMostrarTodo');
btnMostrarTodo.addEventListener('click', totalListTipoVideo);

function totalListTipoVideo() {
  let objXHR;
  ...
  objXHR.open(
    "GET",
    "/admin/practicas/mi_app/acciones/tipo_video/get_tipo_video.php",
    true
  );
  objXHR.send();
  objXHR.onreadystatechange = function () {
    if (objXHR.readyState == 4 && objXHR.status == 200) {
      ...
      //console.log(data);
      const tiposVideos = JSON.parse(data);
      let template = "";
      tiposVideos.forEach((tipoVideo) => {
        template += `
                  <tr class="fila">
                    <td>${tipoVideo.id}</td>
                    <td><a>${tipoVideo.tipoVideo}</a></td>                    
      ...
      let printTipoVideos = document.getElementById("tabla_tipo_video");
      printTipoVideos.innerHTML = "";
      printTipoVideos.innerHTML = template;      
      resaltarFila();
    }
  };
}
        ...
        if (element.value === "seleccionar") {
          ...
          for (const keyFila in fila) {
            if (Object.hasOwnProperty.call(fila, keyFila)) {
              ...
              filaElement.children[2].children[0].addEventListener("click",() => {                  
      ...
      element.addEventListener("click", () => {
        if (element.value === "eliminar") {
          ...
          for (const keyFila in fila) {
            if (Object.hasOwnProperty.call(fila, keyFila)) {
              const filaElement = fila[keyFila];
              filaElement.children[2].children[1].addEventListener("click",() => {
...
function seleccionarRegistro(filaID){
  const inputId = document.getElementById('tipo_video_id');
  inputId.setAttribute("type", "hidden")
  let XHR = new XMLHttpRequest();
  XHR.open("POST", "/admin/practicas/mi_app/acciones/tipo_video/get_id_tipo_video.php")
  XHR.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  XHR.send(`tipo_video_id=${filaID}`);
  XHR.onreadystatechange = function(){
    let rpta = XHR.responseText;
...
function eliminarRegistro(filaID){
  let XHR = new XMLHttpRequest();
  XHR.open("POST", "/admin/practicas/mi_app/acciones/tipo_video/eliminaTipoVideo.php")
  XHR.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  XHR.send(`tipo_video_id=${filaID}`);
  XHR.onreadystatechange = function(){
    let rpta = XHR.responseText;
...
function getRegistro(rptJSON) {
  let inputId = document.getElementById("tipo_video_id");
  let inputTipoNombre = document.getElementById("tipo_video_nombre");

  rptJSON.forEach((rpt) => {
    inputId.value = rpt.id;
    inputTipoNombre.value = rpt.tipoVideo;
  });
}
...
function limpiarInput() {
  const tipoVideo = document.getElementById("tipo_video_nombre");
  tipoVideo.value = "";
}
