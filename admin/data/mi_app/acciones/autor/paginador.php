<?php 
    ...
    $columns = ['autor_id', 'autor_nombre', 'autor_canal', 'autor_url', 'autor_web', 'autor_twitter', 'autor_facebook', 'autor_twitch', 'autor_discord', 'autor_file'];
	  $table = "autor";
	  $id = 'autor_id';
      ...
      /*CONSULTA PARA TOTAL DE REGISTROS FILTRADOS*/
       ...
       if ($num_rows > 0) {
         while ($row = mysqli_fetch_assoc($result)) {
           ...
           $output['data'] .= '<td class="td-border">'.$row['autor_id'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_nombre'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_canal'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_url'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_web'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_twitter'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_facebook'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_twitch'].'</td>'; 
           $output['data'] .= '<td class="td-border">'.$row['autor_discord'].'</td>';
           $output['data'] .= '<td class="td-border">'.$row['autor_file'].'</td>';
        ...
        if ($output['totalRegistros'] > 0) {
         ...
         for ($i=$numeroInicio; $i <= $numeroFin ; $i++) {
           if ($pagina == $i) {
             ...
             $output['paginacion'] .= '<li class="page-item"><a class="page-link" href="#" onclick="getDataAutor('. $i .')">' . $i .'</a></li>';
           }
         }
