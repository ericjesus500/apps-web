...
const models = [
  { model: "lab", first: 50, limits: [-125, -100, -75, -50, -25, 0, 25, 50, 75, 100, 125] },
  { model: "oklab", first: 0.5, limits: [-0.4, -0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4] }
];

const createBoxes = (container, model, first, limits) => {
  container.style.setProperty("--elements", limits.length);
  for (let y = 0; y < limits.length; y++) {
    for (let x = 0; x < limits.length; x++) {
      const div = document.createElement("div");
      div.classList.add("box");
      div.dataset.title = `${model}(${first} ${limits[x]} ${limits[y]})`;
...
for (let p = 0; p < 2; p++) {
  ...
  createBoxes(container, models[p].model, models[p].first, models[p].limits);
...
.color-models .container {
  ...
  grid-template-rows: 45px repeat(var(--elements), 35px);
